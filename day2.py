...
def upper(left_rgb, right_rgb) -> RGB:
    ...
    return RGB(red = max(left_rgb.red, right_rgb.red),
...
def to_rgb(text) -> RGB:
    ...
    dictionary = {'red': 0, 'green': 0, 'blue': 0} | {colour: int(number) for (number, colour) in pairs}
...
class Game:
    ...
    def possible(self, rgb) -> bool:
        return self.minimums.red <= rgb.red and self.minimums.green <= rgb.green and self.minimums.blue <= rgb.blue
